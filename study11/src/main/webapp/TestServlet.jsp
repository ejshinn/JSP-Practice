...
<body>
<%-- 접속 순서 --%>
<%-- 1. TestServlet.jsp 파일로 접속 --%>
<%-- 2. jsp 파일의 내용을 클라이언트에 출력 --%>
<%-- 3. a 태그 클릭 (/TestServlet.do로 이동) --%>
<%-- 4. 설정 파일(web.xml) 확인 --%>
<%-- 5. web.xml 파일의 <servlet-mapping> 태그의 자식 태그 중 <url-pattern> 패턴의 내용과 a 태그에서 href 속성으로 지정한 위치가 같은지 확인 --%>
...
<%-- 9. 요청 시 사용한 통신방식(get, post)에 따라 doGet(), doPost() 메서드를 호출 --%>
...
<div class="container mt-5">
    ...
    <p>
        <%-- request 영역에 저장된 message라는 이름의 변수 가져오기 --%>
        <%-- 현재 페이지를 웹 브라우저에 url을 직접 입력하여 접속 시 request에 저장된 데이터를 가져올 수 없음 --%>
        <strong><%=request.getAttribute("message")%></strong>

        <br>

        <%-- tomcat 서버로 해당 주소를 요청 -> web.xml 파일에 등록된 서블릿의 url을 검색 -> 해당 url을 가지고 있는 서블릿 이름과 동일한 서블릿 이름을 검색 -> 해당 서블릿 이름을 사용하고 있는 클래스 파일을 확인하고 해당 클래스 파일의 doGet()/doPost()를 호출 --%>
        <a href="/TestServlet.do" target="_blank" class="btn btn-link">web.xml에서 설정한 서블릿 바로 가기</a>
    </p>
