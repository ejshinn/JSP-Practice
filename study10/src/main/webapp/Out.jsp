...
<div class="container mt-5">
    <%-- JSTL의 set을 사용하여 변수 선언, value 속성 대신 set의 시작 태그와 끝 태그 사이에 값을 입력해도 됨 --%>
    <c:set var="iTag">
        i 태그는 <i>기울임</i>을 표현합니다.
    </c:set>

    <h3>기본 사용</h3>
    <%-- escapeXml 속성값이 기본으로 true이기 때문에 특수기호를 모두 문자열로 출력 --%>
    <c:out value="${iTag}"></c:out>

    <hr>

    <h3>escapeXml 속성 false</h3>
    <%-- escapeXml 속성이 false이면 특수기호의 기능을 사용함 --%>
    <c:out value="${iTag}" escapeXml="false"></c:out>

    <hr>

    <h3>default 속성</h3>
    <%-- value 속성에 값이 없을 경우 default 속성 값을 대신 사용 --%>
    <%-- 현재 페이지를 서버에 요청 시 name이라는 파라미터를 제공하지 않음, param.name은 null임 --%>
