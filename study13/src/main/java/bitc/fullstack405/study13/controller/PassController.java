...
public class PassController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // view 파일 지정 후 forward()를 사용하여 화면 이동
        req.getRequestDispatcher("/MVCBoard/Pass.jsp").forward(req, resp);
    }

    // Pass.jsp에서 form을 통해 post 방식으로 접속했으므로 doPost()를 호출
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 가져온 결과가 true면 mode 확인, false면 이전 화면으로 이동
        if (confirmed) {
            // 결과가 true면 mode를 확인하여 수정이나 삭제 로직을 실행함
            if (mode.equals("update")) {
                // mode가 update면 /mvcboard/edit.do를 사용하는 서블릿으로 이동
                resp.sendRedirect("/mvcboard/edit.do?idx=" + idx);
            }
            // mode가 delete면 파일 게시물 삭제 로직 실행
            else if (mode.equals("delete")) {
                // 파일 삭제를 위해 파일이 저장된 경로를 가져옴
                String saveDir = this.getServletContext().getInitParameter("FileUploadDirectory");
                // 파일 삭제를 위해 DB에서 현재 게시물의 정보를 가져옴
            ...
            JSFunction.alertBack("비밀번호 검증에 실패했습니다.", resp);
        }
    }
}