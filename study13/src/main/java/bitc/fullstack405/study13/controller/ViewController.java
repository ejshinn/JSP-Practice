...
public class ViewController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 게시물 확인 시 조회수를 업데이트하기 위해 dao에서 제공하는 updateVisitCount()를 호출
        dao.updateVisitCount(idx);

        // 게시물의 상세 정보를 dao에서 제공하는 selectBoardDetail()을 호출하여 가져옴
        ...
        // DB에서 가져온 정보를 request 영역에 board라는 이름으로 저장
        req.setAttribute("board", board);
        // view 페이지인 /MVCBoard/View.jsp를 설정하고 forward()를 사용하여 화면 이동
