...
// @MultipartConfig : 클라이언트에서 form 태그를 통해서 enctype을 multipart/form-data로 설정 후 데이터 전송 시 전달받을 데이터의 크기를 설정하는 애너테이션
// 서버로 전달되는 데이터 타입은 byte 타입임
// 컴퓨터는 2진수로 숫자를 표현하므로 정수 1000을 2의 10승으로 표시하여 1024로 표현함
// 1024 * 1024 * 1 => 1000 * 1000 * 1 => 1,000,000 * 1 => 1MB가 됨
...
public class UploadProcess extends HttpServlet {
    ...
    // 클라이언트에서 전달한 정보 및 데이터베이스 등록을 위한 메서드
    ...
    private void insertMyFile(HttpServletRequest req, String originalFileName, String saveFileName) {
        ...
        // checkbox로 데이터를 전달하여 데이터가 배열로 전달되기 때문에 getParameterValues()를 사용하여 배열로 가져옴
        String[] cateArr = req.getParameterValues("cate");
        StringBuffer cateBuff = new StringBuffer();

        // checkbox로 전달된 데이터가 없을 경우
        if (cateArr == null) {
            cateBuff.append("선택한 항목 없음");
        }
        else {
            // checkbox로 전달된 데이터가 있으면 반복문으로 모두 가져옴
