...
public class MyFileDAO extends JDBConnect {
    ...
    public int insertFile(MyFileDTO myFile) {
        ...
        try {
            ...
            // PreparedStatement를 사용하여 DB에 sql문 전달 및 결과 가져오기
    ...
    public int insertFileList(List<MyFileDTO> myFileList) {
        ...
        try {
            ...
            // values 뒤에 데이터가 들어가는 부분을 반복 입력하여 하나의 insert문으로 생성
            for (int i = 0; i < myFileList.size(); i++) {
                // 마지막 부분은 ','가 없으므로 파일 정보 목록의 크기와 현재 반복수를 비교하여 마지막인지 확인하고 ','를 붙일지 말지 결정
            ...
            // ? 부분의 index 번호를 계속 변경해야 하기 때문에 count라는 변수를 하나 선언하고 setXXX()를 사용할 때마다 값을 증가시킴
            int count = 0;
            for (int i = 0; i < myFileList.size(); i++) {
