...
<head>
    ...
    <title>PreparedStatement로 update</title>
...
<div class="container mt-5">
    <h2 class="text-center">PreparedStatement로 데이터 수정하기</h2>

    <%
        // 데이터베이스로 sql 쿼리문을 전달하는 Statement 클래스 타입의 변수 생성
        Statement stmt = null;
        // sql update문 생성
        // 자바 소스 코드 내에서 sql 쿼리문을 직접 사용할 경우 sql 쿼리문의 문자열이 길기 때문에 += 연산자를 사용하여 여러 줄의 sql 쿼리문을 연결해서 사용하게 됨
        // 문자열을 연결해서 사용 시 문자열의 끝에 공백 기호를 하나 추가하는 것이 좋음
        String sql = "UPDATE member SET pass = '" + userPw + "', name = '" + userName + "' ";
        sql += "WHERE id = '" + userId + "' ";

        // 데이터베이스는 자바 프로그램의 외부에 존재하는 리소스이므로 반드시 try ~ catch를 사용해야 함
        try {
