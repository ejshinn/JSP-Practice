...
<%-- Statement를 사용하여 여러 개의 데이터를 한번에 입력 시 --%>
<%--<%--%>
<%--    try {--%>
<%--        // for 문을 사용하여 Statement 객체를 10번 생성--%>
...
<%-- 여러 개의 데이터를 PreparedStatement로 입력 시(1번 방식) --%>
<%--<%--%>
<%--    try {--%>
...
<%-- 여러 개의 데이터를 PreparedStatement로 입력 시(2번 방식) --%>
<%
    PreparedStatement pstmt = null;
    String sql = "INSERT INTO member (id, pass, name, regidate) ";
    sql += "VALUES (?, ?, ?, now()) ";

    // clearParameters() : PreparedStatement를 사용하여 여러 번 데이터를 수정하여 입력 시 setXXX() 메서드를 사용하여 ?에 입력한 데이터를 삭제
    // addBatch() : PreparedStatement를 사용하여 여러 번 데이터를 수정하여 입력 시 executeUpdate()로 여러 번 데이터베이스로 sql문을 전달하는 것이 아닌 메모리상에 저장해놓고 executeBatch() 호출 시 한번에 전달하는 방식
