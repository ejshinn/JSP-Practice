...
<body>
<%-- exception : jsp가 미리 생성해 놓은 예외 처리 객체 --%>
<%-- page 지시어를 사용하여 errorPage에 설정해 놓은 페이지로 이동시키고, errorPage로 지정된 jsp 파일에 page 지시어 isErrorPage=true 일 때 exception 객체를 사용할 수 있음 --%>
<%-- web.xml에 error-page 속성을 설정하면 page 지시어에 errorPage 속성 및 isErrorPage=true를 설정하지 않아도 오류 발생 시 모든 jsp 페이지에서 web.xml에서 설정한 오류 처리 페이지가 열림 --%>
<%
